...
print(type('t'))
#plt.show()
plt.savefig('M0209_BHmass.png', dpi=300)

WD = './Bhmass_plot/'
fig1, ax1 = plt.subplots()
ax1.set_xlabel('$z$')
#ax.set_xlim(0,4)
ax1.set_ylabel('$log(MBH/Msolar)$')
c=[]
halo=[]
color =10
i=2
...
#for j in range(len(time[i])):
 #ax1.plot(j['t'],'b',j[str])
halo.append(ax1.scatter(time[i], mass[i], s=6, c=c, vmin=0, vmax=100,label=halo_names[i]))
#ax.scatter(time, rat, s=6, c=c, vmin=0, vmax=100)
ax1.legend(handles=halo)

print(type('t'))
#plt.show()
plt.savefig(WD+'M0209_Met.png', dpi=300)